...
void AquariumView::Initialize(wxFrame* parent)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &AquariumView::OnAddClownFish, this, IDM_ADDFISHCLOWN);
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &AquariumView::OnAddGoldeen, this, IDM_ADDFISHGOLDEEN);
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &AquariumView::OnAddDecorCastle, this, IDM_ADDCASTLEDECOR);
...
/**
 * Menu hander for Add Fish>Clown Fish
 * @param event Mouse event
 */
void AquariumView::OnAddClownFish(wxCommandEvent& event)
{
    auto fish = make_shared<ClownFish>(&mAquarium);
    mAquarium.Add(fish);
    Refresh();
}
 ...
 * @param event Mouse event
 */
void AquariumView::OnAddGoldeen(wxCommandEvent& event)
{
    auto fish = make_shared<Goldeen>(&mAquarium);
    mAquarium.Add(fish);
    Refresh();
}
...
void AquariumView::OnLeftDown(wxMouseEvent &event)
{
    ...
    if (mGrabbedItem != nullptr)
    {
        ...
        // Move it to the end of the list of item
        mAquarium.RemoveAddBack(mGrabbedItem);
...
/**
 * Timer for every time aquarium needs to refresh
 * @param event Mouse event
 */
